...
describe("fullContextFetchfullContextFetchMiddleware", () => {
  ...
  it("should take a cache", function(done) {
    const cache = new Cache({ store: "memory", ttl: 1 /*seconds*/ });
    ...
    instance(reqStub, {}, err => {
      ...
      reqStub.hull.cache.set("connector", newShip).then(arg => {
        instance(reqStub, {}, () => {
          expect(reqStub.hull.connector.private_settings.value).to.equal(
            "test2"
          );
          expect(this.getStub.withArgs("app", sinon.match.any).calledOnce).to.be
            .true;
          done();
        });
      });
  ...
  //       expect(this.getStub.withArgs("app", sinon.match.any).calledTwice).to.be
  //         .true;
  //       done();
  //     });
  //   });
  ...
  //   sinon
  //     .stub(req.hull.client, "configuration")
  //     .returns({ id: "foo", secret: "bar", organization: "localhost" });
  //   sinon
  //     .stub(req2.hull.client, "configuration")
  //     .returns({ id: "foo2", secret: "bar2", organization: "localhost2" });

  //   const userSegmentsGetStub = sinon
  //     .stub(req.hull.client, "get")
  //     .withArgs("app", sinon.match.any)
  //     .resolves({
  //       id: "foo"
  //     })
  ...
  //   const userSegmentsGetStub2 = sinon
  //     .stub(req2.hull.client, "get")
  //     .withArgs("app", sinon.match.any)
  //     .resolves({
  //       id: "foo2"
  //     })
  ...
  //       expect(req.hull.usersSegments).to.eql([
  //         { id: "s1", name: "segment 1" }
  //       ]);
  //       expect(req.hull.accountsSegments).to.eql([
  //         { id: "as1", name: "account segment 1" }
  //       ]);

  //       expect(userSegmentsGetStub2.callCount).to.equal(1);
  //       expect(accountSegmentsGetStub2.callCount).to.equal(1);
  //       expect(req2.hull.usersSegments).to.eql([
  //         { id: "s2", name: "segment 2" }
  //       ]);
  //       expect(req2.hull.accountsSegments).to.eql([
  //         { id: "as2", name: "account segment 2" }
  //       ]);
