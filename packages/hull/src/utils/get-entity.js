...
const getAccountSegments = async (ctx, id): Promise<Array<HullAccountSegment>> => {
  if (!id) return [];
...
const getEntity = async (
  ...
  const { data = [] } = await ctx.client.post(`search/${entityType}_reports`, query);
