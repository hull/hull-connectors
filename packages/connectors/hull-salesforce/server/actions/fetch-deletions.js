...
const { fetchDeletedRecords } = require("../lib/sync-agent/actions/incoming/fetch/fetch-deleted-records");
...
async function fetchAction(ctx: HullContext): HullExternalResponse {
  ...
  await fetchDeletedRecords({ privateSettings, syncAgent })
    .catch((err) => {
      ctx.client.logger.error("incoming.job.error", { job: "fetchDeleted", message: err.message, status: err.status });
    });
