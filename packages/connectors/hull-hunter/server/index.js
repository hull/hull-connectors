...
export default async function handle({ request, client, connector }, message) {
  ...
  if (!first_name_attribute || !last_name_attribute) {
    throw new SkippableError(
      "First and last name must be mapped in the connector settings."
    );
  }

  if (!domain_attribute && !company_attribute) {
    throw new SkippableError(
      "Domain or company name must be mapped in the connector settings."
    );
  }

  const valuePayload = { ...user, account };

  const payload = ["domain", "company", "first_name", "last_name"].reduce(
    (acc, propertyName) => {
  ...
  if (!payload.domain && !payload.company) {
    throw new SkippableError(
      "Domain or company name not present on this user."
    );
  }
  const userClaims = {
  ...
  try {
    ...
    if (response.status === 200 && response.body) {
      if (response.body.data && response.body.data.email) {
        ...
        client.asUser(userClaims).logger.warn("outgoing.user.warning", {
          warning:
            "Hunter.io returned no email for this user, saving enriched_at attribute and skipping."
        });
      }
      const traits = {
    ...
    return client.asUser(userClaims).logger.error("outgoing.user.error", {
      error:
        "Hunter.io returned unknown API response, if error persist contact Hull support."
    });
  } catch (error) {
