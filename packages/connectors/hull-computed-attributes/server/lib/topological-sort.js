...
class TopologicalSort<KeyType, ValueType> {
  ...
  sort(): Map<KeyType, INodeWithChildren<KeyType, ValueType>> {
    ...
    for (let i = 0; i < this.sortedKeysStack.length; i++) {
  ...
  exploreNode(nodeKey: KeyType, explorePath: KeyType[]) {
  ...
  addInternalNode(key: KeyType, node: ValueType) {
    assert.strictEqual(
      this.nodes.has(key),
      false,
  ...
  addMultipleInternalNodes(nodes: Map<KeyType, ValueType>) {
