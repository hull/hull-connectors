...
export default function sendUpdateFactory({ io }: any) {
  return function sendPayload(
    ...
    try {
      ...
      if (!_.size(rooms)) {
        return client.logger.info("outgoing.user.error", {
          message: "no rooms"
        });
      }
      ...
      return client
        .asUser(user)
        .logger.info("outgoing.user.success", { rooms, payload });
    } catch (error) {
      return client.asUser(user).logger.error("outgoing.user.error", { error });
    }
  };
}
