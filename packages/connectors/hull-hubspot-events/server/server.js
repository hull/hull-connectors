...
function server(app: $Application, deps: Object): $Application {
  ...
  app.post("/fetchAllEmailEvents", jsonHandler({
    callback: (req: HullRequest): Promise<any> => {
      return hullRouter.incomingRequest("fetchAllEmailEvents", req);
    },
    options: {
      fireAndForget: true
    }
  ...
  app.post("/fetchRecentEmailEvents", jsonHandler({
    callback: (req: HullRequest): Promise<any> => {
      return hullRouter.incomingRequest("fetchRecentEmailEvents", req);
    },
    options: {
      fireAndForget: true
    }
