  ...
  return testScenario({ connectorServer }, ({ handlers, nock, expect }) => {
    ...
    return _.assign(updateMessages, {
      ...
      externalApiMock: () => {
        ...
        const batchPayloadScope = nock("https://hull-platform.s3.amazonaws.com");
        batchPayloadScope
          .get("/extracts/user_report/report.json")
          .reply(200, require("../fixtures/notifier-payloads/outgoing-user-batch-multiple-payload.json"));

        const scope = nock("https://api.outreach.io");
        scope
          .get("/api/v2/prospects/16")
          .reply(401, require("../fixtures/api-responses/outgoing-user-batch-401.json"));
        scope
          .get("/api/v2/prospects/18")
          .reply(401, require("../fixtures/api-responses/outgoing-user-batch-401.json"));
        scope
        ...
        return [scope, batchPayloadScope];
      },
      response: {
        flow_control: {
          type: "next",
