...
test("fetch all accounts and prospects from outreach and retry on failure", () => {
  return testScenario({ manifest, connectorConfig, debounceWait: 3000 }, ({ handlers, nock, expect }) => {
    return {
      ...
      logs:  [
        ...
        ["debug", "connector.service_api.call", {}, {"method": "GET", "responseTime": expect.whatever(), "status": 500, "url": "/accounts/", "vars": {}}],
        ["error", "incoming.job.error", {}, {"error": "The Outreach system has thrown an error.  Hull has tried to recover, but was unable.  Please contact your Hull Service Representative so that they can contact Outreach and inform them of this bug", "jobName": "Incoming Data Request"}]
      ],
      firehoseEvents: [
