...
test("send smart-notifier user update to outreach", () => {
  return testScenario({ connectorConfig }, ({ handlers, nock, expect }) => {
    const updateMessages = _.cloneDeep(require("../fixtures/notifier-payloads/outgoing-account-changes-account-attribute-user-level.json"));
    // set this as an id so that we know it exists and doesn't get sent just because there's no id
    // specifically check for the account id
    updateMessages.messages[0].user["outreach/id"] = 23;
    ...
    return _.assign(updateMessages, {
      ...
      externalApiMock: () => {
        ...
        scope
          .intercept('/api/v2/prospects/23', 'PATCH', {"data":{"type":"prospect","id":23,"attributes":{"custom1":"newdomain.com"}}})
      ...
      logs: [
        ...
        ["info", "outgoing.user.success", expect.objectContaining({"request_id": expect.whatever(), "subject_type": "user", "user_email": "alberto@close.io", "user_id": "userid"}), { "data": {"data": {"attributes": { "custom1": "newdomain.com" }, "id": 23, "type": "prospect"}},
        type:"Prospect" }],
