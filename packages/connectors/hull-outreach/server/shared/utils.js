...
function toSendMessage(
  ...
  if (!_.isEmpty(entityInSegments)) {
    ...
    if (targetEntity === "user") {
      ...
      context.client.asUser(entity).logger.info("outgoing.user.skip", {
        reason: `User is not present in any existing segment (${segmentAttribute}).  Please add the user to an existing synchronized segment`
      });
    } else if (targetEntity === "account") {
      debug(`Account is not a part of any segment ${JSON.stringify(entity)}`);
      context.client.asAccount(entity).logger.info("outgoing.account.skip", {
        reason: `Account is not present in any existing segment (${segmentAttribute}).  Please add the account to an existing synchronized segment`
      });
    }
    return false;
  }
